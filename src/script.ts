...
let selection: MapSelection = new MapSelection(mapState);
...
image.onload = function () {
    for (let i = 0; i < 20; i++) {
        ...
        for (let j = 0; j < 16; j++) {
            ...
            let tile = new SpriteSquare(canvas, context, i, j, selection);
            row.push(tile);
        }
    ...
    for (let i = 0; i < 20; i++) {
        ...
        for (let j = 0; j < 16; j++) {
            ...
            spritesContainer.appendChild(canvas);
            let tile = new SpriteSquare(canvas, context, i + 20, j + 16, selection);
            row.push(tile);
        }
        spriteSquares.push(row);
    }
